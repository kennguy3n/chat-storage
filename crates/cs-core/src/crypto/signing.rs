...
//!
//! To maintain security under both classical and post-quantum adversaries
//! we sign each canonical manifest payload **twice**:
//!
//! * **Ed25519** — fast, well-audited classical EdDSA.
//! * **ML-DSA-65** — FIPS 204 module-lattice signature, NIST Category 3.
//!
...
/// Hybrid signing key bundling an Ed25519 key with an ML-DSA-65 key.
#[derive(Clone)]
...
impl HybridSigningKey {
    ...
    /// Generate a fresh hybrid signing key from `rng`.
    ...
    /// Borrow the ML-DSA-65 verifying component cached at generation time.
    pub fn ml_dsa_verifying_key(&self) -> &MlDsaVerifyingKey {
        &self.ml_dsa_verifying
    }

    /// Derive the matching [`HybridVerifyingKey`] for this signing key.
    ...
    /// Sign `payload` with both component keys and return the two signatures.
    pub fn sign_payload(&self, payload: &[u8]) -> CryptoResult<(Ed25519Signature, MlDsaSignature)> {
        ...
        let mldsa = self
            ...
            .map_err(|_| CryptoError::Signature("ml-dsa-65 sign failed"))?;
        Ok((ed, mldsa))
    }
}
...
impl HybridVerifyingKey {
    ...
    pub fn verify_ed25519(&self, payload: &[u8], signature: &[u8]) -> CryptoResult<()> {
        ...
        self.ed25519
            .verify(payload, &sig)
            .map_err(|_| CryptoError::Signature("manifest: ed25519 verify failed"))
    }
    ...
    pub fn verify_ml_dsa(&self, payload: &[u8], signature: &[u8]) -> CryptoResult<()> {
        ...
        self.ml_dsa
            .verify(payload, &sig)
            .map_err(|_| CryptoError::Signature("manifest: ml-dsa-65 verify failed"))
    }

    /// Verify **both** signatures over the same payload.
...
/// Diagnostic returned when one of the legs fails.
