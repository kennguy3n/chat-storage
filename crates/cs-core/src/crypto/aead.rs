...
//! Authenticated Data. Pattern C (ZK Object Fabric) does *not* use
//! this AAD — see [`crate::crypto::convergent`] for that codepath.
...
use super::{CryptoError, CryptoResult, Key32, Nonce12, Nonce24};
...
/// Object class encoded in [`build_kchat_chunk_aad`].
...
// ---------------------------------------------------------------------------
// Top-level convenience functions (backward-compatible with existing callers)
// ---------------------------------------------------------------------------

/// Seal plaintext with XChaCha20-Poly1305.
///
/// Returns `ciphertext || tag` (the combined output from the AEAD).
pub fn seal(key: &Key32, nonce: &Nonce24, plaintext: &[u8], aad: &[u8]) -> CryptoResult<Vec<u8>> {
    xchacha20_poly1305::seal(key, nonce, plaintext, aad)
}

/// Open (decrypt) XChaCha20-Poly1305 ciphertext.
pub fn open(key: &Key32, nonce: &Nonce24, ciphertext: &[u8], aad: &[u8]) -> CryptoResult<Vec<u8>> {
    xchacha20_poly1305::open(key, nonce, ciphertext, aad)
}
...
pub fn seal_aes(
    ...
    aes_256_gcm::seal(key, nonce, plaintext, aad)
}
...
pub fn open_aes(
    ...
    aes_256_gcm::open(key, nonce, ciphertext, aad)
}
...
pub mod xchacha20_poly1305 {
    ...
    pub fn seal(
        ...
        cipher
            ...
            .map_err(|_| CryptoError::Aead("xchacha20-poly1305 seal failed".to_string()))
    }
    ...
    pub fn open(
        ...
        cipher
            ...
            .map_err(|_| CryptoError::Aead("xchacha20-poly1305 open failed".to_string()))
    }
}
...
pub mod aes_256_gcm {
    ...
    pub fn seal(
        ...
        cipher
            ...
            .map_err(|_| CryptoError::Aead("aes-256-gcm seal failed".to_string()))
    }
    ...
    pub fn open(
        ...
        cipher
            ...
            .map_err(|_| CryptoError::Aead("aes-256-gcm open failed".to_string()))
    }
}

// ---------------------------------------------------------------------------
// KChat per-chunk AAD construction
// ---------------------------------------------------------------------------
