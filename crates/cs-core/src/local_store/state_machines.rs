...
//!
//! Each enum here matches the state diagrams in the design docs exactly:
//!
...
//!   `snake_case` strings the schema columns use.
...
///
/// The terminal states ([`BodyState::DeletedForMe`],
/// [`BodyState::DeletedForEveryone`], [`BodyState::Unavailable`])
...
impl BodyState {
    /// Whether `to` is reachable from `from` per the state diagram.
...
///
/// Skeleton-first restore: the restore pipeline visits every state
/// in declaration order. The terminal state is
